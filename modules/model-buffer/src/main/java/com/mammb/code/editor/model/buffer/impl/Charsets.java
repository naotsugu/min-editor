...
public class Charsets {
    ...
    /** The size of buffer. */
    private static final int bufSize = 8_000;
     ...
     */
    public static Charset estimateCharset(Path path) {
        return estimateCharset(path, 0, bufSize);
    }
     ...
     * Estimate the charset.
     * @param path the source path
     * @param sampleSize the sampling byte size
     * @return the charset of estimated
     */
    public static Charset estimateCharset(Path path, long skipNBytes, int sampleSize) {

        try (var is = Files.newInputStream(path)) {

            is.skipNBytes(skipNBytes);
     ...
     * @return the charset
     */
    public static Charset estimateCharset(InputStream source, int sampleSize) {

        final InputStream is = source.markSupported()
            ? source
