...
public class TextEditorModel implements EditorModel {
    ...
    public void imeOff() {
        content.clearFlush();
        carets.getPrimaryOne().clearImeFlush();
    }

    @Override
    public boolean isImeOn() {
        return carets.getPrimaryOne().hasImeFlush();
    }
    ...
    public TextEditorModel with(Session session) {
        ...
        model.carets.getPrimaryOne().at(row, Math.clamp(session.caretCol(), 0, model.screenLayout.rowTextAt(row).textLength()));
    ...
    public void apply(Action action) {
        ...
        decorate.clearFlushMarks();
        BracketFind.apply(carets.getPrimaryOne().point(), query(Query.charAtCaret), screenLayout.screenRows()).forEach(p ->
    ...
    public <R> R query(Query<R> query) {
        return switch (query) {
            case QueryRecords.CaretPoint _        -> (R) carets.getPrimaryOne().point();
            ...
            case QueryRecords.CharAtCaret _       -> (R) Contents.lrTextAt(content, carets.getPrimaryOne().point());
            case QueryRecords.BytesAtCaret _      -> (R) Contents.bytesAt(content, carets.getPrimaryOne().point());
