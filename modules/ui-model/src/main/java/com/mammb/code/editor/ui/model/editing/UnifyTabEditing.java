 ...
 */
public class UnifyTabEditing implements Editing {
     ...
     */
    private UnifyTabEditing(Input input) {
        this.input = input;
    }
    ...
    public static Editing of(String text) {
        return "\t".equals(text)
            ? new UnifyTabEditing(new Input(text))
            : Editing.empty;
    }
    ...
    public boolean apply(EditorModel model) {
        var text = model.query(ModelQuery.caretBeforeText);
