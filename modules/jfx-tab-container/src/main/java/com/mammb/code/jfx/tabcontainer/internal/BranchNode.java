...
public class BranchNode extends TreeNode implements ParentOf<TreeNode> {
    ...
    public void add(ContentPane content, LeafNode source, Side side) {
        ...
        if (children().size() <= 1) {
            ...
            splitPane.setDividerPositions(prevDivider);
        } else {
            var prevDivider = splitPane.getDividerPositions();
            removeChild(source);
    ...
    private void balance() {
        ...
        if (isRoot() && children.isEmpty()) {
            if (ctx.stages().size() == 1) {
                addChildren(List.of(new LeafNode(ctx, ctx.handlers().requireContent())));
            } else {
